...
Snapshot GameEngine::snapshot() const {
    Snapshot result;
    result.version = 5; result.mode = mode_; result.rules = rules_; result.phase = phase_; result.currentPlayer = currentPlayer_; result.winner = winner_; result.turn = turn_; result.rngState = random_.state();
...
bool GameEngine::restore(const Snapshot& snapshot) {
    if (snapshot.version < 2 || snapshot.version > 5 || snapshot.players.size() < 2 || snapshot.players.size() > 5 || snapshot.territories.size() != 42) return false;
